...
function listarClientes() {
    fetch('https://localhost:5000/api/Cliente/ListadodeClientes')
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            ...
            tablaCliente.innerHTML = ''; // Limpiar tabla antes de volver a llenarla

            clientes.forEach(item => {
                const fila = document.createElement('tr');
                fila.id = `cliente-${item.idcliente}`; // Agregar un ID único a la fila
                fila.innerHTML = `
                    <td>${item.nombre}</td>
                    ...
                    <td>${item.estado}</td>
                    <td>
                        <button class="btn-editar" onclick="editarCliente(${item.idcliente})">
                            <i class="fa fa-edit"></i>
                        </button>
                        <button class="btn-eliminar" onclick="eliminarCliente(${item.idcliente})">
